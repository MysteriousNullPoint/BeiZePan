        ...
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ExportData demoBean = (ExportData) objList.get(j);
                    List<String> list = new ArrayList<>();
                    String type=demoBean.getType();
                    if(type.equals("库存")){
                        list.add(demoBean.getName());
                        list.add(demoBean.getLocation());
                        list.add(demoBean.getRemain());
                        list.add(demoBean.getStock());
                        list.add(demoBean.getUsed());
                        list.add(demoBean.getScrapped());
                    }

                    else if(type.equals("模板")){
                        list.add(demoBean.getName());
                        list.add(demoBean.getClass_name());
                        list.add(demoBean.getStock());
                        list.add("0");
                    }
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        if (list.get(i).length() <= 4) {
                            //设置列宽
                            sheet.setColumnView(i, list.get(i).length() + 20);
                        } else {
                            //设置列宽
                            sheet.setColumnView(i, list.get(i).length() + 15);
                        }
                    }
                    //设置行高
